function varargout = cs_spm_get(varargin)
...
CmdLine = cs_spm('CmdLine',CmdLine);
...
if CmdLine>0
	...
	%---------------------------------------------------------------
	P = cs_spm_get('CmdLine',n,char(Prompt),[],NewWDir);
	...
	%---------------------------------------------------------------
	%-Set up FileSelWin
	[F,cF] = cs_spm_get('Initialise','on',n,char(Prompt),Filter,NewWDir);
...
if (exist('cs_spm.m')==2), LastDirs=strvcat(LastDirs,cs_spm('Dir')); end
...
%-----------------------------------------------------------------------
WS = cs_spm('WinScale');				%-Window scaling factors
FS = cs_spm('FontSizes');				%-Scaled font sizes
PF = spm_platform('fonts');			%-Font names (for this platform)
S0 = get(0,'ScreenSize');			%-Screen size

F  = figure('IntegerHandle','off',...
	'Tag','SelFileWin',...
	'Name',sprintf('%s%s: SPMget',cs_spm('ver'),cs_spm('GetUser',' (%s)')),...
	'NumberTitle','off',...
...
if strcmp(spm_platform('filesys'),'win')
	...
	uicontrol(F,'Style','PopUp','Tag','DrivesPopup',...
		...
		'HorizontalAlignment','Left',...
		'ForegroundColor','r',...
		'String',cs_spm_get('DrivesPullDownStr'),...
		'Callback',['cs_spm_get(''cd''),'...
		    'set(gcbo,''String'',cs_spm_get(''DrivesPullDownStr''))'],...
...
if exist('spm_help.m')==2
	uicontrol(F,'Style','Pushbutton','String','?',...
		'ToolTipString','help on using cs_spm_get',...
		'ForegroundColor','g',...
		'Callback','spm_help(''cs_spm_get.m'')',...
...
set(F,'KeyPressFcn',...
	'cs_spm_get(''FigKeyPressFcn'',get(gcbf,''CurrentCharacter''),gcbf)')
...
if any(abs(ch)==[10,11,19]) % ^J,^K,^S
	...
	if abs(ch)==19 & hPos
		%- ^S - Select
		switch get(h,'Tag')
			case 'IName', cs_spm_get('Add',h)
			case 'SelIName', cs_spm_get('Delete',h)
			case 'DirName', cs_spm_get('cd',get(h,'UserData'))
		end
		return
	end
...
elseif abs(ch)==13	%- Return - Goto topic / apply filter
	if EditWDir, cs_spm_get('cd',str)
		else, cs_spm_get('dir'), end
	return
...
case {'off','reset'}
	...
	return
case 'on'
	if isempty(F), 	F=cs_spm_get('CreateFig'); end	%-Create SelFileWin
	varargout = {F,cF};				%-Return figure handles
	...
	set(findobj(F,'Tag','Prompt'),...
		...
		'UserData',n)
							%-Reset prompt, & n
	cs_spm_get('StatusLine',0,n,F)			%-Reset StatusLine
	...
	if ~isempty(WDir)
		cs_spm_get('cd',WDir)			%-Change directory
	else
		cs_spm_get('dir')				%-Show current dir.
	end
...
%-----------------------------------------------------------------------
NewDir = cs_spm_get('CPath',NewDir,WDir);
...
if size(Dirs,1)
	...
	h = text(0,y,deblank(Dirs(i,:)),...
		...
		'ButtonDownFcn','cs_spm_get(''cd'',get(gcbo,''UserData''))',...
...
if ~NoComp & size(Items,1)>32
	...
	if Filter(end)=='*'
		[IName,ItemPos] = cs_spm_get('FileSummary',Items);
	else
		[IName,ItemPos] = cs_spm_get('FileSummary',Items,'front',Filter);
	end
	text(0.45,y0,'Summary View - Click to expand',...
		'Parent',hAxes,...
		'Color','w',...
		'FontSize',cs_spm('FontSize',9),...
		'HorizontalAlignment','Center',...
		'FontAngle','Italic',...
		'ButtonDownFcn','cs_spm_get(''dir'',[],[],1)',...
		'HandleVisibility','off');
else
...
if size(IName,1)
	...
	h = text(0.4,y,cIName,...
		...
		'UserData',cItems,...
		'Color','k',...
		'ButtonDownFcn','cs_spm_get(''Add'')',...
		'HandleVisibility','on');
	if nItems>1;
		h = text(0.39,y-3,int2str(sum(ItemPos(i,:)>0)),...
			'Parent',hAxes,...
			'Color','w',...
			'FontSize',cs_spm('FontSize',8),...
			'FontAngle','Italic',...
			'HorizontalAlignment','right',...
			'UserData',cIName,...
			'ButtonDownFcn',...
			'cs_spm_get(''dir'',[],get(gcbo,''UserData''),1)',...
			'HandleVisibility','off');
	end
	y = y - dy;
    end
...
if size(S,2)<=3
	...
	tS            = S;
	tS(:,1:3)     = [];
	[null,tI]     = cs_spm_get('StrSort',tS);
	[null,hI]     = cs_spm_get('StrSort',hS(tI,:));
	I             = tI(hI);
	sS            = S(I,:);	
end
varargout = {sS,I};
...
elseif strcmp(Cend,'end')
	fFnames           = cs_spm_get('strfliplr',Fnames);
	fFilter           = fliplr(Filter);
	[FSpecs,FnamePos] = cs_spm_get('FileSummary',fFnames,'front',fFilter,len);
	FSpecs            = cs_spm_get('strfliplr',FSpecs);
elseif strcmp(Cend,'both')
	[hSpecs,hI] = cs_spm_get('FileSummary',Fnames,'front');
	FSpecs      = '';
	FnamePos    = [];
	for i = 1:size(hSpecs,1)
		...
		[tSpecs,tI] = cs_spm_get('FileSummary',Cfnames,'end',Filter,len);
...
for h = H'
	...
	set(h,'String',tmp,'Color','b',...
		'Tag','SelIName',...
		'ButtonDownFcn','cs_spm_get(''Delete'')')
end
...
%-Update StatusLine
%-----------------------------------------------------------------------
cs_spm_get('StatusLine',size(P,1),n,F)
...
PNew = cs_spm_get('CmdLine',n,Prompt,P,WDir,1);

if ~strcmp(P(:),PNew(:))
	set(findobj(F,'Tag','P'),'UserData',PNew);
	cs_spm_get('dir')
	cs_spm_get('StatusLine',size(PNew,1),n,F)
end
...
if (nP<n.mn | nP>n.mx)
	...
	cs_spm('Beep')
	msgbox(str,sprintf('%s%s: %s...',cs_spm('ver'),...
		cs_spm('GetUser',' (%s)'),mfilename),'error','non-modal')
	drawnow
...
case 'cpath'
%=======================================================================
% cpath = cs_spm_get('cpath',path,cwd)
if nargin<3, cwd=pwd; else, cwd=cs_spm_get('CPath',varargin{3}); end
...
%=======================================================================
% str = cs_spm_get('DrivesPullDownStr')
