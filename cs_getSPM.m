...
% Modified by MRJ for CenterScripts where noted.
SCCSid = '2.51';
global cstemp;  %added by MRJ
%-GUI setup
%-----------------------------------------------------------------------
...
%-----------------------------------------------------------------------
%swd   = spm_str_manip(spm_get(1,'SPM.mat','Select SPM.mat'),'H'); commented out by MRJ
swd    = pwd;   %MRJ: assumes we are in a stats dir when called
%-Preliminaries...
%=======================================================================
...
% 		'	Select contrasts...',' for conjunction',1);     commented out by MRJ
Ic          = cstemp.con;                                   % added by MRJ
...
% else
	...
	pm = [];
	Ex = [];
% end       all those lines in the 'Get contrasts for masking' section commented out by MRJ
...
%titlestr    = spm_input('title for comparison','+1','s',str);     commented out by MRJ
titlestr     = ['CenterScripts automatic display, contrast ',int2str(cstemp.con)];
...
%     switch spm_input('p value adjustment to control','+1','b',str,[],1)       commented out by MRJ
    switch 'none'                                                           % added by MRJ
	...
	% p for conjunctions is p of the conjunction SPM
   	%---------------------------------------------------------------
	u  = cstemp.pval;   %spm_input(['threshold {',STAT,' or p value}'],'+0','r',0.001,1);   changed/commented out by MRJ
...
if ~isempty(XYZ) & length(Ic) == 1
    ...
    %-------------------------------------------------------------------
    %k     = spm_input('& extent threshold {voxels}','+1','r',0,1,[0,Inf]);         commented out by MRJ
    k=0;                                                                            %line added by MRJ
