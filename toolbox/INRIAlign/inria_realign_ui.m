...
if (nargin == 0)
    ...
    for i = 1:n,
        if strcmp(MODALITY,'FMRI'),
            ...
            for s=1:ns,
                ...
                while size(p,1)<1,
                    if isempty(BCH),
                        ...
                        p = spm_select(Inf, 'Image', ['scans for subj ' num2str(i) ', sess' num2str(s)]);
                    else,
            ...
            p{1} = '';
            while size(p{1},1)<1,
                p{1} = spm_select(Inf, 'Image', ['select scans for subject ' num2str(i)]);
                %p{1} = spm_get(Inf,'.img',...
    ...
    if (WhchPtn == 1 | WhchPtn == 3) & sptl_WghtRg,
        if spm_input(...
            ...
            if isempty(BCH),
                ...
                PW = spm_select(n, 'Image', 'Weight images for each subj');
            else,
